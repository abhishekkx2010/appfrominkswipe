...
/**
 * Created by Ajinkya Deshpande on 8/24/16.
 */
...
public class CalendarUtils {
    ...
    public static boolean isTodayDay(Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.set(Calendar.HOUR_OF_DAY, 0);
        calendar.set(Calendar.MINUTE, 0);
        calendar.set(Calendar.SECOND, 0);
    ...
    public static boolean isMonthSame(Date date) {
        Calendar calendar = Calendar.getInstance();
        calendar.set(Calendar.MONTH, 0);
        LoggerGeneral.info("showmonthchanged333---" + date.getMonth() + "---" + Calendar.MONTH);

        if (date.getMonth() == calendar.get(Calendar.MONTH)) {

            LoggerGeneral.info("showmonthchanged444---"+date.getMonth()+"---"+Calendar.MONTH);
            return true;
        }
